...
var tool = {
    ...
    getMyAjax: function (url, data, callback) {
    ...
    getFormeditor: function (str) {
        ...
        if (xx('type="button"') != -1) {
            ...
            if (formeditor.search('id="add"') != -1) {//判断是否有添加  添加行列按钮功能
            ...
            if (formeditor.search('id="move"') != -1) {//判断是否有添加 移动按钮
            ...
            if (formeditor.search('idtype="shu"') != -1) {//判断是否有树结构
            ...
            if (formeditor.search('idtype="file"') != -1) {//判断是否有上传功能
            ...
            if (formeditor.search('idtype="total"') != -1) {//判断是否有合计功能
            ...
            if (formeditor.search('idtype="paging') != -1) {//判断是否有分页功能
                var randomNum = parseInt(Math.random() * 1000);
                var pagingConfig = '{' +
                ...
                var myStr09 = '<style>#paging' + randomNum + '{text-align:center;margin: 10px 0 20px;}#paging' + randomNum + '>ul,#paging' + randomNum + '>ul>li,#paging' + randomNum + '>div{display:inline-block;margin:0;padding:0;list-style:none;vertical-align:middle}#paging' + randomNum + '>ul>li{float:left;color:#08c;padding:4px 10px;border:1px solid #ddd;cursor:pointer}#paging' + randomNum + '>ul>li:hover,#paging' + randomNum + ' a:hover{color:#005580;background-color:#f5f5f5}#paging' + randomNum + '>ul>li:first-child{border-radius:4px 0 0 4px}#paging' + randomNum + '>ul>li:last-child{border-radius:0 4px 4px 0}#paging' + randomNum + ' a{display:inline-block;font-size:14px;line-height:20px;text-decoration:none;padding:4px 10px;border:1px solid #ddd;border-radius:4px;vertical-align:middle}.pagingavtive{background-color:#f5f5f5;color:#999!important;cursor:default!important}#paging' + randomNum + '>div{vertical-align:top}#paging' + randomNum + ' input{margin:0;width:20px;vertical-align:top}#paging' + randomNum + '>div>input{border-radius:4px 0 0 4px}#paging' + randomNum + '>div>a{border-radius:0 4px 4px 0}</style>' +
                    ...
                    '<script>pagingFnNav(' + pagingConfig + ');function pagingFnNav(obj){var url=obj.url;var pagingNum,ispaging,showPaging=obj.showPaging||5;var pagingFn=document.getElementById(obj.id);var pagingHome=document.getElementById(obj.pagingHome);var pagingLast=document.getElementById(obj.pagingLast);var pagingGo=document.getElementById(obj.pagingGo);pagingHome.onclick=function(){fn(this.getAttribute("data-pagingnum"))};pagingLast.onclick=function(){fn(this.getAttribute("data-pagingnum"))};pagingGo.onclick=function(){var numV=this.previousElementSibling.value;this.previousElementSibling.value="";fn(numV)};pagingFn.onclick=function(ev){var ev=ev||window.event;var target=ev.target||ev.srcElement;if(target.nodeName.toLowerCase()=="li"){if(target.getAttribute("class")=="pagingavtive"){return false}fn(target.getAttribute("data-pagingnum"))}};fn();function fn(pageNum){pagingFnAjax(url+"?pageNum="+(pageNum||1)+"&pageSize="+(obj.pageSize||10),function(data){pagingFn.innerHTML="";var num=0;pagingNum=data.paging;ispaging=data.ispaging;pagingHome.setAttribute("data-pagingnum",1);pagingLast.setAttribute("data-pagingnum",pagingNum);var intN=Math.floor(showPaging/2)+1;var remainPageNum=pagingNum-ispaging;var navPageNum=showPaging-intN;if(ispaging>intN&&remainPageNum>=navPageNum){num=ispaging-intN}else{if(remainPageNum<navPageNum){num=pagingNum-showPaging;if(remainPageNum<=0){pagingLast.setAttribute("class","disabled")}}}for(var i=num;i<showPaging+num;i++){var elL=document.createElement("li");elL.setAttribute("data-pagingnum",i+1);elL.innerHTML=i+1;if(ispaging==i+1){elL.setAttribute("class","pagingavtive")}pagingFn.appendChild(elL)}})}function pagingFnAjax(url,callback){var xhr=new XMLHttpRequest();xhr.open("get",url);xhr.onreadystatechange=function(){var XMLHttpReq=xhr;if(XMLHttpReq.readyState==4){if(XMLHttpReq.status==200){var data=XMLHttpReq.response;data=JSON.parse(data);callback(data)}else{if(XMLHttpReq.status==404){alert("404")}}}};xhr.send()}};</script>';
                formeditor = formeditor.replace(/<input\s[^>]+idtype="paging".*?>/g, myStr09);
            }
            if (formeditor.search('idtype="table"') != -1) {//判断是否有表格功能
            ...
            if (formeditor.search('id="download"') != -1){//判断是否有下载功能
                // debugger;
                // console.log('下载按钮');
                var downUrl = './data/download.json';
                formeditor +='<script>var downloads=document.querySelectorAll(\'[idtype="download"]\');for(var i=0;i<downloads.length;i++){downloads[i].onclick=function(){var body=document.querySelector("body").innerHTML;var url=" ' + downUrl + ' ";var xhr=new XMLHttpRequest();xhr.open("POST",url,true);xhr.setRequestHeader("Content-type","application/x-www-form-urlencoded");xhr.send({data:body});xhr.onreadystatechange=function(){var XMLHttpReq=xhr;if(XMLHttpReq.readyState==4){if(XMLHttpReq.status==200){var data=JSON.parse(XMLHttpReq.response);window.open(data.url)}else{alert("下载功能------"+XMLHttpReq.status)}}}}};<\/script>';
            }
